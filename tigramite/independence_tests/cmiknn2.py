...
class CMIknn(CondIndTest):
    ...
    **kwargs :
    ...
    def _get_nearest_neighbors(self, array, xyz, knn):
        ...
        tree_xyz = KDTree(array.T, 16, metric="minkowski", p=np.inf)
        distances, _ = tree_xyz.query(array.T, k=knn + 1)
    ...
    def get_shuffle_significance(self, array, xyz, value,
        ...
        if len(z_indices) > 0 and self.shuffle_neighbors < T:
            ...
            tree_xyz = KDTree(z_array, metric="minkowski", p=np.inf)
            neighbors = tree_xyz.query(z_array, k=self.shuffle_neighbors, )[1].astype('int32')
