...
class PCMCI_Parallel:
    ...
    def run_pc_stable_parallel_singleVariable(self, variables):
        allSelectedLinks = {key: deepcopy(self.__allSelectedLinks[key]) for key in self.__allSelectedLinks.keys()}
        out = []
        pcmci_var = PCMCI(dataframe=pp.DataFrame(self.__data.copy()), cond_ind_test=self.__cond_ind_test())
        for variable in variables:
            ...
            parents_of_var, otherStats = pcmci_var.run_pc_stable_single_var(variable, tau_min=self.__tau_min,
                                                                            ...
                                                                            selected_links=allSelectedLinks,
                                                                            otherReturn=False)
            print(f"PC algo done for var {variable}, time {time.time() - start} s")
            parents_of_var = {var: deepcopy(parents_of_var[var]) for var in parents_of_var.keys()}
            out.append([variable, pcmci_var, parents_of_var, otherStats, pcmci_var.selected_links, pcmci_var.temp_dict])
        return out
    ...
    def start(self, nbWorkers: int = None):
        ...
        for elem in pc_output:
            for innerElem in elem:
                ...
                self.temp_dict.update(innerElem[-1])
                self.selected_links.update(innerElem[-2])
        ...
        for pc_out in pc_output:
            for innerOut in pc_out:
                index = innerOut[0]
                self.val_min.update({index: innerOut[3]["val_min"]})
                self.pval_max.update({index: innerOut[3]["pval_max"]})
...
if __name__ == '__main__':
    ...
    pcmci_par2 = PCMCI_Parallel(data, ParCorr, 0, 5, 0.01)
    start = time.time()
    results_pcmci_par2 = pcmci_par2.start()
    print(f"Total time: {time.time() - start}")
    print(pcmci_par2.temp_dict == seq_pcmci.temp_dict)
    exit()
