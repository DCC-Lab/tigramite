...
class NewVariant_PCMCI_Parallel_ForTests:
    def __init__(self, data: np.ndarray, tau_min: int, tau_max: int, pc_alpha: float):
        self.__nbVar = data.shape[-1]
        self.__data = pp.DataFrame(data)
        self.__cond_ind_test = ParCorr()
...
if __name__ == '__main__':
    ...
    print("New variant (no copies)")
    par_new = NewVariant_PCMCI_Parallel_ForTests(data, 1, 5, 0.01)
    start = time.time()
    par_new.start()
    print(f"Total time: {time.time() - start} s")
    print("New variant (with copies)")
